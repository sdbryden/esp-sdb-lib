...
static char *s_hostname = "192.168.1.20";
static int s_port = 8000;
static char *s_filename = "test.bin";
...
{
    ESP_LOGI(TAG, "Server IP: %s Server Port:%d", s_hostname, s_port);
    ...
    sock_info.sin_addr.s_addr = inet_addr(s_hostname);
    sock_info.sin_port = htons(s_port);
...
static void ota_example_task(void *pvParameter)
{
    ...
    int get_len = asprintf(&http_request, GET_FORMAT, s_filename, s_hostname, s_port);
    if (get_len < 0) {
...
void ota_do_upgrade(char *hostname, int port, char *filename)
{
    s_hostname = hostname;
    s_port = port;
    s_filename = filename;
